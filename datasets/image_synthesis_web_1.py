...
def fusion_image (i, image, label_text):
  ...
  if i <= 0:
    ...
    # Read craters image
    if i <= 2:
      image_crater = load_image(crater_path1)
    else:
    ...
    while True:
      if i <= 2:
        image_crater = resize_img(image_crater,2)
        cols, rows = image_crater.shape[:2]
        resized_image = rotate_image(image_crater, None)
        y = random.randint(98, 1013)
        x = random.randint(7379, 7620)
      else:
        # random crater size
...
if __name__ == '__main__': 
  ...
  bg_src = 'itri_shadow1' #web/itri/itri_shadow
  ...
  for mod in modes:
    ...
    crater_path1 = os.path.join(crater_dir, mod, bg_remove+'1')
    ...
    elif bg_src == 'itri_shadow1':
      image_path = "20240420_bg5.jpg"
    else:
      image_path = "Airport-plane-runway-top-view_3840x2160.jpg"
      
    for i in tqdm.tqdm(range(start, fusion_fin_number)):
