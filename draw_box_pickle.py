...
LABEL_FOLDER = './mmdetection/results_mmdet/results_itri_bg_cropped.pkl'  # Put the label files in this folder. 
RAW_IMAGE_FOLDER = './datasets/fusion_image/test/images'  # Put the original images without boxes in this folder. 
OUTPUT_IMAGE_FOLDER = './mmdetection/results_mmdet/images_cropped_labeled/'  # The output images would be saved to this folder
IMAGE_NAME_LIST_PATH = './datasets/fusion_image/test/images_croped.txt'  # The file name of images will be saved into this text file.
...
def draw_box_on_image(i, image_name, colors, LABEL_FOLDER, RAW_IMAGE_FOLDER, OUTPUT_IMAGE_FOLDER):
    """
    ...
    source_file = pickle.load(open(LABEL_FOLDER,'rb'))
...
if __name__ == '__main__':           
    ...
    f = open(IMAGE_NAME_LIST_PATH, 'r')
    lines = f.read()
    f.close()
    names = lines.split('\n')[:-1]
    
    for i,image_name in enumerate(names):
        box_num = draw_box_on_image(i, image_name, colors, LABEL_FOLDER, RAW_IMAGE_FOLDER, OUTPUT_IMAGE_FOLDER)  
        box_total += box_num
        image_total += 1
