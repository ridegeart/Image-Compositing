...
def print_map_summary(mean_ap,
    ...
    precisions = np.zeros((num_scales, num_classes), dtype=np.float32)  #定?
    ...
    header = ['class', 'gts', 'dets', 'recall', 'precision', 'ap']  #打印precision??
    for i in range(num_scales):
        if scale_ranges is not None:
        ...
        table_data.append(['mAP', '', '', '', '', f'{mean_ap[i]:.3f}'])  #多加?''??
