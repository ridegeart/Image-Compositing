...
from datasets.joint_image import joint_main
import glob
import tqdm
import os
import argparse
...
def predict(path):
    ...
    for image_name in tqdm.tqdm(os.listdir(path)):
        ...
        new_width = crop_dataset(imgpath, windows_width, windows_height, annotation, savePath)

    #windows_width = new_width
    ...
    result = joint_main(path, txtpath, yolo_labels_path, windows_width, windows_height,
               scale=1)
    return result
...
if __name__ == '__main__':
    path = '/home/training/datasets/fusion_image/val/images_bg'
    predict(path)